...
@Component({
  selector: 'kt-laporan-buku-besar-rev',
  templateUrl: './laporan-buku-besar-rev.component.html',
  styleUrls: ['./laporan-buku-besar-rev.component.scss', '../laporan.style.scss']
})
export class LaporanBukuBesarRevComponent implements OnInit, AfterViewInit {
  // View Child List
  ...
  setPreparation() {
    if (this.kodePerusahaan != '') {
      ...
      let cabangAksesSts = false, akunAksesSts = false, periodeTranSts = false, // Status Request Data
      ...
      this.request.apiData('cabang', 'g-cabang-akses', reqData).subscribe(
        data => {
          if (data['STATUS'] === 'Y') {
            ...
            if (cabangAksesSts && akunAksesSts && periodeTranSts) {
              this.setContent()
            }
          } else {
      ...
      // Request Data Akun
      this.request.apiData('akun', 'g-group-akun-user', reqData).subscribe(
        data => {
          if (data['STATUS'] === 'Y') {
            this.ref.markForCheck()
            this.listGroupAkunUser = data['RESULT']
            akunAksesSts = true

            // Set List Akun
            this.listAkunDupl = JSON.parse(JSON.stringify(this.listGroupAkunUser)) // Akun Duplicate
  ...
  actionData() {
    ...
    this.formInputValFinal['list_akun'] = xAkun
    ...
    if (this.reportResultId[this.rptRecycleId] == undefined) {
      ...
      this.request.apiData('report', 'g-data-buku-besar', reqData).subscribe(
        data => {
          if (data['STATUS'] === 'Y') {
            this.ref.markForCheck()
  ...
  getReport(type, res?, urlRpt?) {
    ...
    this.rptRecycleId = this.formInputValFinal.format_laporan
      ...
      + this.formInputValFinal['list_akun'].toString()
