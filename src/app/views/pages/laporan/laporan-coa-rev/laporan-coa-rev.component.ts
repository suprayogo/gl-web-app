...
@Component({
  selector: 'kt-laporan-coa-rev',
  templateUrl: './laporan-coa-rev.component.html',
  styleUrls: ['./laporan-coa-rev.component.scss', '../laporan.style.scss']
})
export class LaporanCoaRevComponent implements OnInit, AfterViewInit {
  ...
  createRpt(params, formatRpt) {
    this.keyReportFormatExcel = params['rep_id']
    this.urlReport['sheets'] = params['rep_url']
    if (formatRpt === 'xlsx') {
      this.keyReportFormatExcel = this.keyReportFormatExcel + '.xlsx'
      setTimeout(() => {
        let sbmBtn: HTMLElement = document.getElementById('fsubmit') as HTMLElement;
        sbmBtn.click();
      }, 100)
    } else {
      this.keyReportFormatExcel = this.keyReportFormatExcel + '.xls'
      setTimeout(() => {
        let sbmBtn: HTMLElement = document.getElementById('fsubmit') as HTMLElement;
        sbmBtn.click();
      }, 100)
    ...
    this.checkKeyReport[rk] = params['rep_id']
    this.ref.markForCheck()
    this.loading = false
  }
