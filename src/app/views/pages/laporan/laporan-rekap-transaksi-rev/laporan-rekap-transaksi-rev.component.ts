...
@Component({
  selector: 'kt-laporan-rekap-transaksi-rev',
  templateUrl: './laporan-rekap-transaksi-rev.component.html',
  styleUrls: ['./laporan-rekap-transaksi-rev.component.scss', '../laporan.style.scss']
})
export class LaporanRekapTransaksiRevComponent implements OnInit, AfterViewInit {
  ...
  onSubmit(inputForm: NgForm) {
    if (this.forminput !== undefined) {
      ...
      let
        ...
        // KEY REPORT
        rk = this.formValue['tipe_laporan'] + this.formValue['kode_cabang'] + this.formValue['kode_bank'] + this.formValue['no_rekening'] + tgl_periode_awal + tgl_periode_akhir + this.formValue['format_laporan'] + xAkun.toString()
  ...
  getReport(repKey, fPeriod, lPeriod) {
    ...
    if (this.checkKeyReport[repKey] !== undefined) {
      if (this.formValue['format_laporan'] === 'pdf') {
        window.open(this.urlReport['pdf'] + '?repId=' + this.checkKeyReport[repKey], '_blank')
      } else {
      ...
      this.request.apiData('report', 'g-data-rekapitulasi', endRes).subscribe(
        data => {
          if (data['STATUS'] === 'Y') {
            if (this.formValue.format_laporan === 'pdf') {
              this.urlReport['pdf'] = data['RESULT']['rep_url']
              window.open(this.urlReport['pdf'] + "?repId=" + data['RESULT']['rep_id'], '_blank')
            } else {
              this.urlReport['sheets'] = data['RESULT']['rep_url']
              if (this.formValue.format_laporan === 'xlsx') {
                this.keyReportFormatExcel = data['RESULT']['rep_id'] + '.xlsx'
                setTimeout(() => {
                  let sbmBtn: HTMLElement = document.getElementById('fsubmit') as HTMLElement
                  sbmBtn.click();
                }, 100)
              } else {
                this.keyReportFormatExcel = data['RESULT']['rep_id'] + '.xls'
                setTimeout(() => {
                  let sbmBtn: HTMLElement = document.getElementById('fsubmit') as HTMLElement
                  sbmBtn.click();
                }, 100)
            ...
            this.ref.markForCheck()
            let rk = this.formValue['tipe_laporan'] + this.formValue['kode_cabang'] + this.formValue['kode_bank'] + this.formValue['no_rekening'] + fPeriod + lPeriod + this.formValue['format_laporan'] + this.formValue['list_akun'].toString()
            this.checkKeyReport[rk] = data['RESULT']['rep_id']
            this.loading = false
          } else {
            this.ref.markForCheck()
            this.loading = false
