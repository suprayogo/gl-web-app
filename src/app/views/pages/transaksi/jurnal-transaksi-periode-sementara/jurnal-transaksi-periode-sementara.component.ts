...
const content = {
  beforeCodeTitle: 'Jurnal Transaksi Buka Kembali'
}
...
export class JurnalTransaksiPeriodeSementaraComponent implements OnInit, AfterViewInit {
  ...
  inputLayout = [
    {
      ...
      blurOption: {
        ...
        onFound: () => {
          this.inputTanggalData = this.daftar_periode_kasir.filter(x => x['kode_cabang'] === this.forminput.getData()['kode_cabang'] && x['buka_kembali'] === '1')
        }
      },
      update: {
        disabled: true
  ...
  openDialog(type) {
    if (type === 'tgl_periode') {
      ...
      if (this.forminput.getData()['kode_cabang'] === "") {
        this.gbl.openSnackBar('Pilih cabang dahulu.', 'info', () => {
          setTimeout(() => {
      ...
      data: {
        ...
        tableInterface:
          type === "kode_cabang" ? this.inputCabangInterface :
            type === "tgl_periode" ? this.inputTanggalInterface :
              {},
        displayedColumns:
          type === "kode_cabang" ? this.inputCabangDisplayColumns :
            type === "tgl_periode" ? this.inputTanggalDisplayColumns :
              [],
        tableData:
          type === "kode_cabang" ? this.inputCabangData :
            type === "tgl_periode" ? this.inputTanggalData :
              [],
        tableRules:
          type === "kode_cabang" ? this.inputCabangDataRules :
            type === "tgl_periode" ? this.inputTanggalDataRules :
              [],
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        if (type === "kode_cabang") {
          if (this.forminput !== undefined) {
            if (result.kode_cabang !== this.forminput.getData()['kode_cabang']) {
              this.forminput.updateFormValue('kode_cabang', result.kode_cabang)
              this.forminput.updateFormValue('nama_cabang', result.nama_cabang)
              this.forminput.updateFormValue('tgl_tran', '')
      ...
      this.request.apiData('divisi', 'g-divisi', { kode_perusahaan: this.kode_perusahaan }).subscribe(
        data => {
          if (data['STATUS'] === 'Y') {
            ...
            this.gbl.updateInputdata(data['RESULT'], 'kode_divisi', this.inputLayout)
  ...
  refreshBrowse(message) {
    if (this.forminput.getData()['kode_cabang'] !== '') {
      if (this.forminput.getData()['tgl_tran'] !== '') {
        this.tableLoad = true
        ...
        this.request.apiData(
          ...
          'g-jurnal-transaksi',
          {
            ...
            tgl_periode: this.forminput.getData()['tgl_tran']
          }
        ).subscribe(
          data => {
            if (data['STATUS'] === 'Y') {
              if (message !== '') {
                this.browseData = data['RESULT'].filter(x => x['kode_cabang'] === this.forminput.getData()['kode_cabang'])
                this.loading = false
        ...
        this.openSnackBar('Pilih tanggal terlebih dahulu.', 'info', () => {
          this.selectedTab = 0
          this.ref.markForCheck()
        })
      }
