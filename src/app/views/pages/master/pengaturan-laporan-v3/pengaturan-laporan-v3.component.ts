...
export class PengaturanLaporanV3Component implements OnInit {
    ...
    onSubmit(type) {
      ...
      if (type === 'lbrg') {
        ...
        console.log('Data from getData() in formLBValue:', res); // Tambahkan ini
        ...
        console.log('Data from getData() in formNRValue:', res); // Tambahkan ini
        res['kode_perusahaan'] = this.kode_perusahaan;
        ...
        console.log('Data from getData() in formAKValue:', this.formAKValue); // Tambahkan ini
...
--------------------------------------------------------------*/
    ...
    openDialog(type, st, ft) {
      console.log('Argumen kode_akun (type):', type);
      console.log('Argumen lbrg (ST):', st);
      console.log('Argumen type (FT):', ft);
        ...
        screenPst =
          ft === 'pn' || ft === 'hpp' || ft === 'al' || ft === 'at' || ft === 'ak' || ft === 'psv' ? lv1 :
            ft === 'pu' || ft === 'bp' || ft === 'atb' || ft === 'hl' || ft === 'bdp' || ft === 'bp' ? lv2 :
              ...
              this.gbl.screenPosition(screenPst)
              console.log('this gbl screen :',screenPst);
              console.log('this gbl TOP :',topPst);
              console.log('Nilai this.inputAkunTableRules:', this.inputAkunTableRules);
              console.log('Isi dari LBSetting sebelum perubahan:', this.LBSetting);
              console.log('Nilai this.inputAkunData:', this.inputAkunData);
              

// FILTER PADA SAAT OPEN DIALOG
const filterdata = st === "lbrg" ?
this.inputAkunData.filter(input => {
return !this.LBSetting['daftarPengakuanNilai'][ft].some(akun => akun.kode_akun === input.kode_akun);
}) :
st === "nr" ?
this.inputAkunData.filter(input => {
return !this.NRSetting['daftarPengakuanNilai'][ft].some(akun => akun.kode_akun === input.kode_akun);
}) :
st === "ak" ?
this.inputAkunData.filter(input => {
return !this.AKSetting['daftarPengakuanNilai'][ft].some(akun => akun.kode_akun === input.kode_akun);
}) :
[];
...
// console.log(filterdata);
      ...
      const dialogRef = this.dialog.open(DialogComponent, {
        ...
        data: {
          ...
          tableData: type === "kode_akun" ? (
            st === "lbrg" ? (ft in this.LBSetting['daftarPengakuanNilai'] ? filterdata.filter(x => !this.selectedLBAkun.includes(x['kode_akun'])) : []) :
            st === "nr" ? (ft in this.NRSetting['daftarPengakuanNilai'] ? filterdata.filter(x => !this.selectedLBAkun.includes(x['kode_akun'])) : []) :
            st === "ak" ? (ft in this.AKSetting['daftarPengakuanNilai'] ? filterdata.filter(x => !this.selectedLBAkun.includes(x['kode_akun'])) : []) :
            this.inputAkunData
        ) : [],
  ...
  // Console log data yang dilewatkan ke dialog
  console.log('Data yang dilewatkan ke dialog:', {
    type: type,
    tableInterface: {},
  ...
  dialogRef.afterClosed().subscribe(result => {
    console.log('Dialog ditutup dengan hasil:', result);
    ...
    if (result) {
      ...
      console.log('Hasil dialog diubah menjadi objek x:', x);
  
      if (type === "kode_akun") {
        if (st === "lbrg") {
          console.log('Menambahkan ke LBSetting dan selectedLBAkun');
          this.loadingLB = true;
          this.ref.markForCheck();
          ...
          console.log('Menambahkan ke NRSetting dan selectedNRAkun');
          this.loadingNR = true;
          this.ref.markForCheck();
          ...
          console.log('Menambahkan ke AKSetting dan selectedAKAkun');
          this.loadingAK = true;
          this.ref.markForCheck();
    ...
    console.log('Isi dari LBSetting setelah perubahan:', this.LBSetting);
    ...
    delDetailData(e, st, ft) {
      console.log('Argumen  (e):', e);
      console.log('Argumen  (ST):', st);
      console.log('Argumen (FT):', ft);
